...
# -*- coding: utf-8 -*-
"""
Created on Sat Dec 31 18:04:13 2016
...
def myplot(dirout,lon,lat,rgname,typstr,datatype,u,v,data,iz,plv):
    ...
    if typstr=='diff':
        titstr='Differences in wind and moisture between the wet and dry conditions at '+levstr
    else:
...
dirin='D:/Work//MyPaper/PhD02/Pics/'
dirout='D:/Work//MyPaper/PhD02/Pics/Patterns_MJJAS/'
regions=['ETP','WTP']
ng=len(regions)
...
for ip in range(0,ncls):
    ...
    if ip<2:
        for ig in range(0,ng):
            filename='MJJAS_'+classes[ip]+'_Only_in_'+regions[ig]+"_patterns_out_u_v_q.txt"
        ...
        filename='MJJAS_'+classes[ip]+"_ETP+WTP_patterns_out_u_v_q.txt"
        fpath=dirin+filename
        onedim=readAscii(fpath,0)
        for iz in range(0,nz):
            for ix in range(0,nx):
