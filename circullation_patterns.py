...
# -*- coding: utf-8 -*-
"""
Created on Mon Sep 26 17:00:34 2016
    ...
    if year[i]>=1979 and month[i]>4 and month[i]<10:
        ...
        if wet[0,i]>0:
            ...
            for iz in range(0,nz):
                for ix in range(0,nx):
                    for iy in range(0,ny):            
                        uwet[iz,iy,ix]=uwet[iz,iy,ix]+u[iz,iy,ix]
                        vwet[iz,iy,ix]=vwet[iz,iy,ix]+v[iz,iy,ix]
                        qwet[iz,iy,ix]=qwet[iz,iy,ix]+q[iz,iy,ix]        
###############################################################################
fpath=dirout+"WTP_patterns_out_u_v_q_dry.txt"
foutdry=open(fpath,"w")
fpath=dirout+"WTP_patterns_out_u_v_q_wet.txt"
foutwet=open(fpath,"w")
for iz in range(0,nz):
    for ix in range(0,nx):
        for iy in range(0,ny):            
            uwet[iz,iy,ix]=uwet[iz,iy,ix]/contwet
            item="%e "%(uwet[iz,iy,ix])
            foutwet.write(item)
            vwet[iz,iy,ix]=vwet[iz,iy,ix]/contwet
            item="%e "%(vwet[iz,iy,ix])
            foutwet.write(item)
            qwet[iz,iy,ix]=qwet[iz,iy,ix]/contwet
            item="%e "%(qwet[iz,iy,ix]*1000)
            foutwet.write(item)
            foutwet.write("\n")                        
