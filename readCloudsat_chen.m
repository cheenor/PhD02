...
foldpath='X:\Data\Cloudsat\TP\GEO\'
file=dir('X:\Data\Cloudsat\TP\GEO\*2B-GEOPROF_GRANULE_P*.hdf');
nf=length(file)
%%%%
...
% Read data.
  ...
  [data_rf, status] = hdfsw('readfield', swath_id, DATAFIELD_NAME, [],[],[]);
  DATAFIELD_NAME = 'CPR_Cloud_mask';
  [data_cpr, status] = hdfsw('readfield', swath_id, DATAFIELD_NAME, [],[],[]);
...
% Make type double for plotting.
  ...
  lon=double(lon);
  time=double(time);
  data_rf=double(data_rf);
  data_cpr=double(data_cpr);

% Read attributes.
  [long_name_rf, status] = hdfsw('readattr', swath_id, ...
                         'Radar_Reflectivity.long_name');
  [units_rf, status] = hdfsw('readattr', swath_id, ...
                       'Radar_Reflectivity.units');
  [scale_factor_rf, status] = hdfsw('readattr', swath_id, ...
                               'Radar_Reflectivity.factor');
  scale_factor_rf = double(scale_factor_rf);

  [valid_range_rf, status] = hdfsw('readattr', swath_id, ...
                              'Radar_Reflectivity.valid_range');
  [long_name_cpr, status] = hdfsw('readattr', swath_id, ...
                         'CPR_Cloud_mask.long_name');
  [units_cpr, status] = hdfsw('readattr', swath_id, ...
                       'CPR_Cloud_mask.units');
  [scale_factor_cpr, status] = hdfsw('readattr', swath_id, ...
                               'CPR_Cloud_mask.factor');
  scale_factor_cpr = double(scale_factor_cpr);

  [valid_range_cpr, status] = hdfsw('readattr', swath_id, ...
                              'CPR_Cloud_mask.valid_range');
%
%
...
% Apply scale factor according to [1].
  data_rf = data_rf / scale_factor_rf;
% Apply scale factor according to [1].
  data_cpr = data_cpr / scale_factor_cpr;
...
fileOut='D:\MyPaper\PhD02\Data\EventsProfile_cloudsat.txt';
