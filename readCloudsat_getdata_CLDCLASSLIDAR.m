...
%%%%
varnm={};
varnm{1}='Cloudlayer';  %nray
varnm{2}='CloudLayerBase';
varnm{3}='LayerBaseFlag';
varnm{4}='CloudLayerTop'; 
varnm{5}='LayerTopFlag';
varnm{6}='CloudFraction';
varnm{7}='CloudPhase'; 
varnm{8}='CloudPhaseConfidenceLevel'; %Confidence level assigned to the cloud phase for each layer. 
                                       %It has a value ranging from 0 to 10. 10 indicates the highest 
                                       %confidence level. If confidence level is below 5, use the cloud 
                                       %phase with a caution.
%%%%%%%  LO
varnm{9}='CloudLayerType';  % Cloud type for each layer.
                            %0 = Not determined   %1 = cirrus   %2 = Altostratus
                            %3 = Altocumulus   %4 = St   %5 = Sc   %6 = Cumulus
                            %7 = Nimbostratus Cloud   %8 = Deep Convection
varnm{10}='PrecipitationFlag'; % -1 = not determined   0 = no precipitation
...
for ir=1:2
      ...
      fileOut=strcat(pathout,rgns{ir},'_EventsCloudsat_',SWATHNAME,'_',varnm{ipp},'_',yearstr,'.txt');
      if ipp==1
